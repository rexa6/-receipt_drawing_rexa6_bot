...
token = '7813243186:AAF8RU5m_WO8Fm6BsbTNJ8LvTZr_BCNY4dA' 
...
def main(message):
    if message.text == '📝Отрисовка (Чеки, балансы)':
        ...
        bot.send_message(message.chat.id, "Заказать рекламу: @r1exa6", reply_markup=main_keyboard())
        bot.register_next_step_handler(message, main)
        ...
        if message.text:
            if message.text == "Вернуться в Меню🧭":
                ...
                try:
                    ...
                    if len(text) < 4:
                        bot.send_message(message.chat.id, "Некорректный формат. Пожалуйста, разделите данные переносом строки.", reply_markup=main_keyboard())
                        bot.register_next_step_handler(message, privat)
                        return
                    ...
                    bbox_money = d.textbbox((0, 0), money, font=font_money)
                    w1, h1 = bbox_money[2] - bbox_money[0], bbox_money[3] - bbox_money[1]

                    bbox_balance = d.textbbox((0, 0), balance, font=font_balance)
                    w2, h2 = bbox_balance[2] - bbox_balance[0], bbox_balance[3] - bbox_balance[1]
        ...
        if message.text:
            if message.text == "Вернуться в Меню🧭":
                ...
                try:
                    ...
                    d = ImageDraw.Draw(tink)
                    d.text((70, 25), time_, font=font_time, fill=(225, 238, 229, 255))
        ...
        if message.text:
            if message.text == "Вернуться в Меню🧭":
                ...
                try:
                    ...
                    # Вычисляем размеры текста для суммы
                    bbox_money = font_time.getbbox(money_text)
                    w1 = bbox_money[2] - bbox_money[0]
                    
                    # Вычисляем размеры текста для кэшбэка
                    bbox_cashback = font_time.getbbox(cashback)
                    w2 = bbox_cashback[2] - bbox_cashback[0]
                    
                    d = ImageDraw.Draw(tink)
                    d.text((70, 25), time_text, font=font_time, fill=(195,195,195,255))
                    d.text((193, 125), date_time, font=font_date_time, fill=(255,255,255,255))
                    d.text(((W - w1 - 260) / 2, 755), money_text, font=font_money, fill=(255,255,255,255))
    ...
    if message.text:
        if message.text == "Назад":
            ...
            try:
                ...
                W = 1080
                bbox_money2 = font1.getbbox(money2)
                w1 = bbox_money2[2] - bbox_money2[0]
                bbox_phone = font2.getbbox(phone)
                w2 = bbox_phone[2] - bbox_phone[0]
